...
mem_init(void)
{
    ...
    memset((void *)pages, 0, npages * sizeof(struct PageInfo));
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    // Add by Zhou
    pte_t *result = NULL;
    pte_t *dir_entry_tmp = pgdir + PDX(va);

    if (!(*dir_entry_tmp & PTE_P)) {
        ...
        if (create) {
            ...
            if (new_page != NULL) {
                ...
                new_page->pp_ref++;
                *dir_entry_tmp = page2pa(new_page) | PTE_P | PTE_W | PTE_U;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for (; start < end; start += PGSIZE) {
        ...
        cur_pte = pgdir_walk(env->env_pgdir, (void *)start, 0);

        if (((int)start > ULIM) || (cur_pte == NULL) || (((uint32_t)(*cur_pte) & perm) != perm)) {
